...
function getSession(): User | null {
  try { return JSON.parse(localStorage.getItem('urbanova-session') || 'null') as User | null; } catch { return null; }
}

function saveSession(user: User) { localStorage.setItem('urbanova-session', JSON.stringify(user)); }
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 pb-14 pt-10 sm:gap-10 sm:px-8 sm:pb-16 sm:pt-14 md:grid-cols-[.9fr_1.1fr] md:gap-8 md:pb-24 md:pt-24">
        <div className="relative z-10 animate-rise"><SectionLabel>Public activity, made legible</SectionLabel><h1 className="font-display max-w-xl text-[clamp(3.2rem,8vw,7rem)] font-bold leading-[.9] tracking-[-.07em] text-balance">Your work is already a <span className="text-primary">city.</span></h1><p className="mt-7 max-w-md text-base leading-7 text-muted-foreground sm:text-lg">URBANOVA turns the things you build in public into a living, explorable world. See the signal. Find the shape. Keep moving.</p><div className="mt-8 flex flex-wrap items-center gap-3"><Link href="/demo" data-testid="link-hero-demo" className="flex items-center gap-3 bg-primary px-5 py-3.5 font-mono text-[10px] uppercase tracking-[.14em] text-primary-foreground transition-transform hover:-translate-y-1">Explore a sample city <ArrowUpRight size={15} /></Link><Link href="/how-to" data-testid="link-hero-how-to" className="flex items-center gap-2 border border-border px-5 py-3.5 font-mono text-[10px] uppercase tracking-[.14em] text-muted-foreground hover:border-primary hover:text-primary">Read the field guide <ChevronRight size={15} /></Link></div><div className="mt-7 flex flex-wrap items-center gap-2.5" aria-label="Mobile apps coming soon"><StoreBadge store="apple" /><StoreBadge store="google" /></div><div className="mt-10 flex items-center gap-4 border-t border-border/70 pt-5 font-mono text-[10px] uppercase tracking-[.15em] text-muted-foreground"><span className="text-foreground">01</span><span className="h-px w-12 bg-border" />A map, not a feed</div></div>
          <div className="city-canvas-shell relative min-h-[430px] animate-rise [animation-delay:120ms] sm:min-h-[470px] md:min-h-[540px]"><div className="absolute -inset-12 bg-[radial-gradient(circle,rgba(244,185,78,.13),transparent_55%)]" /><UrbanovaCityPreview /><div className="absolute -bottom-3 right-2 z-20 border border-primary/40 bg-[#12172a]/90 px-3 py-2.5 backdrop-blur-md sm:px-4 sm:py-3"><div className="font-mono text-[8px] uppercase tracking-[.15em] text-primary">City health</div><div className="mt-1 flex items-end gap-2"><span className="font-display text-2xl font-semibold sm:text-3xl">86.4</span><span className="mb-1 font-mono text-[9px] text-muted-foreground sm:text-[10px]">/ 100</span></div></div></div>
      </div>
    </section>
...
function Login() {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setError('Enter a valid email address to continue.'); return; } setError(''); const name = email.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase()); saveSession({ email, name: name || 'City builder', joined: new Date().toISOString() }); setStatus('Opening your command center...'); setTimeout(() => setLocation('/dashboard'), 450); };
  const wallet = async (kind: 'metamask' | 'phantom') => {
    setConnecting(kind); setStatus('');
    try {
      if (kind === 'metamask') {
        ...
        saveSession({ email: '', name: 'Wallet builder', wallet: accounts[0], joined: new Date().toISOString() });
      } else {
        ...
        saveSession({ email: '', name: 'Wallet builder', wallet: result.publicKey.toString(), joined: new Date().toISOString() });
      }
      setStatus('Wallet connected. Opening your command center...'); setTimeout(() => setLocation('/dashboard'), 450);
    } catch (walletError) { setStatus(walletError instanceof Error ? walletError.message : 'Wallet connection was cancelled.'); }
    finally { setConnecting(''); }
  };
  return <div className="noise"><SiteHeader /><main className="mx-auto grid min-h-[calc(100dvh-73px)] max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 md:grid-cols-[1fr_420px] md:py-20"><div className="hidden md:block"><SectionLabel>Private command center</SectionLabel><h1 className="max-w-xl font-display text-6xl font-bold leading-[.92] tracking-[-.06em]">Come in. The lights are already <span className="text-primary">on.</span></h1><p className="mt-7 max-w-md leading-7 text-muted-foreground">Your city is a personal layer over your public work. Sign in to tune its districts, read its health, and pick a direction.</p><div className="mt-10 flex items-center gap-4 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground"><ShieldCheck size={17} className="text-secondary" />No backend account required for this preview</div></div><div className="border border-border bg-card/80 p-6 shadow-[0_25px_80px_rgba(0,0,0,.25)] sm:p-8"><div className="mb-8 md:hidden"><SectionLabel>Private command center</SectionLabel><h1 className="font-display text-4xl font-bold tracking-[-.05em]">Come in. The lights are <span className="text-primary">on.</span></h1></div><div className="mb-6 flex items-center gap-3"><span className="grid size-10 place-items-center border border-primary/60 bg-primary/10 text-primary"><UserRound size={17} /></span><div><h2 className="font-display text-xl font-semibold">Enter your city</h2><p className="font-mono text-[9px] uppercase tracking-[.13em] text-muted-foreground">Local preview access</p></div></div><form onSubmit={submit} noValidate><label htmlFor="email" className="font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Email address</label><input id="email" value={email} onChange={(event) => setEmail(event.target.value)} type="email" placeholder="you@yourcity.dev" className="mt-2 w-full border border-input bg-background px-3 py-3 text-sm text-foreground placeholder:text-muted-foreground/70" data-testid="input-email" aria-describedby={error ? 'login-error' : undefined} />{error && <p id="login-error" className="mt-2 text-xs text-destructive" data-testid="text-login-error">{error}</p>}<button type="submit" data-testid="button-email-login" className="mt-4 flex w-full items-center justify-center gap-2 bg-primary py-3.5 font-mono text-[10px] uppercase tracking-[.15em] text-primary-foreground">{status && !connecting ? status : 'Continue with email'} <ArrowUpRight size={14} /></button></form><div className="my-6 flex items-center gap-3"><span className="h-px flex-1 bg-border" /><span className="font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground">or use a wallet</span><span className="h-px flex-1 bg-border" /></div><div className="grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => wallet('metamask')} disabled={!!connecting} data-testid="button-connect-metamask" className="flex items-center justify-center gap-2 border border-border py-3 font-mono text-[9px] uppercase tracking-[.11em] text-muted-foreground hover:border-primary hover:text-primary disabled:opacity-50"><WalletCards size={14} />{connecting === 'metamask' ? 'Connecting...' : 'MetaMask'}</button><button type="button" onClick={() => wallet('phantom')} disabled={!!connecting} data-testid="button-connect-phantom" className="flex items-center justify-center gap-2 border border-border py-3 font-mono text-[9px] uppercase tracking-[.11em] text-muted-foreground hover:border-primary hover:text-primary disabled:opacity-50"><Orbit size={14} />{connecting === 'phantom' ? 'Connecting...' : 'Phantom'}</button></div>{status && <p className="mt-4 border-l-2 border-primary px-3 py-2 text-xs leading-5 text-muted-foreground" data-testid="status-login">{status}</p>}<p className="mt-7 text-center text-[11px] leading-5 text-muted-foreground">This preview stores a demo session in your browser only. <Link href="/cookies" data-testid="link-login-cookies" className="text-primary hover:underline">Read the policy</Link></p></div></main></div>;
}
